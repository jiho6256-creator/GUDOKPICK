              ...
              <div style={{ background: 'var(--bg)', borderRadius: 12, padding: 18, marginBottom: 18 }}>
                <div className="review-form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                  <div style={{ position: 'relative' }}>
                    ...
                    />
                    {naverUser && <span style={{ position: 'absolute', top: 7, left: 14, fontSize: 11, fontWeight: 700, color: localStorage.getItem('login_type') === 'guest' ? '#6B7280' : localStorage.getItem('login_type') === 'google' ? '#4285F4' : '#03C75A' }}>✓ {localStorage.getItem('login_type') === 'guest' ? '게스트' : localStorage.getItem('login_type') === 'google' ? '구글 로그인' : '네이버 로그인'}</span>}
                  </div>
